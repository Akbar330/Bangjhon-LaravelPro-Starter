...
<aside class="fixed inset-y-0 left-0 z-20 hidden w-80 border-r border-white/60 bg-white/70 px-6 py-8 shadow-[0_18px_60px_rgba(15,23,42,0.08)] backdrop-blur-xl lg:block">
    <div class="flex h-full flex-col">
        <div>
            <p class="text-sm font-medium uppercase tracking-[0.24em] text-sky-600">Bangjhon</p>
            <h1 class="mt-3 text-2xl font-semibold tracking-tight text-slate-950">{{ config('bangjhon-pro-starter.brand.name') }}</h1>
            <p class="mt-2 text-sm leading-6 text-slate-500">Starter UI designed for modern teams and internal products.</p>
        </div>

        <nav class="mt-10 space-y-2">
            <a href="{{ route('bangjhon.dashboard') }}" class="kaizen-nav-link {{ request()->routeIs('bangjhon.dashboard') ? 'is-active' : '' }}">
                User dashboard
            </a>

            @if (($user->role ?? 'user') === 'admin')
                <a href="{{ route('bangjhon.admin.dashboard') }}" class="kaizen-nav-link {{ request()->routeIs('bangjhon.admin.dashboard') ? 'is-active' : '' }}">
                    Admin dashboard
                </a>
                <a href="{{ route('bangjhon.admin.users') }}" class="kaizen-nav-link {{ request()->routeIs('bangjhon.admin.users') ? 'is-active' : '' }}">
                    User management
                </a>
            @endif
        </nav>
